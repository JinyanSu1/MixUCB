...
def plot_six_baselines(Figure_dir='Figures', mixucb_result_postfix="", delta=0.5, result_root=''):
    ...
    for each_perfect_expert_pkl in perfect_expert_pkls:
        with open(os.path.join(result_root, 'perfect_expert_results', each_perfect_expert_pkl), 'rb') as f:
            ...
            raw_rewards = [CR_PerfectExpert[i] - CR_PerfectExpert[i - 1] if i > 0 else CR_PerfectExpert[i] for i in
                           range(len(CR_PerfectExpert))]
    ...
    for each_mixucbI_pkl in mixucbI_pkls:
        with open(os.path.join(result_root, f'mixucbI_results{mixucb_result_postfix}/{delta}', each_mixucbI_pkl),
            ...
            # extract raw rewards from cumulative reward list.
            raw_rewards = [CR_mixucbI[i] - CR_mixucbI[i - 1] if i > 0 else CR_mixucbI[i] for i in
                           range(len(CR_mixucbI))]
    ...
    for each_mixucbII_pkl in mixucbII_pkls:
        with open(os.path.join(result_root, f'mixucbII_results{mixucb_result_postfix}/{delta}', each_mixucbII_pkl),
            ...
            # extract raw rewards from cumulative reward list.
            raw_rewards = [CR_mixucbII[i] - CR_mixucbII[i - 1] if i > 0 else CR_mixucbII[i] for i in
                           range(len(CR_mixucbII))]
    ...
    for each_mixucbIII_pkl in mixucbIII_pkls:
        with open(os.path.join(result_root, f'mixucbIII_results{mixucb_result_postfix}/{delta}', each_mixucbIII_pkl),
            ...
            # extract raw rewards from cumulative reward list.
            raw_rewards = [CR_mixucbIII[i] - CR_mixucbIII[i - 1] if i > 0 else CR_mixucbIII[i] for i in
                           range(len(CR_mixucbIII))]
    ...
    ARII_list = [
        np.array(perfect_expert_rawreward_list[0]) * ~np.array(q, dtype=bool) - np.array(l) * ~np.array(q, dtype=bool)
        for (q, l) in zip(q_mixUCBII_list, mixucbII_rawreward_list)]
    ARII_list = [np.cumsum(l) for l in ARII_list]
    ARIII_list = [
        np.array(perfect_expert_rawreward_list[0]) * ~np.array(q, dtype=bool) - np.array(l) * ~np.array(q, dtype=bool)
        for (q, l) in zip(q_mixUCBIII_list, mixucbIII_rawreward_list)]
