...
warnings.filterwarnings(action='ignore', message=".*Incorrect array format causing data to be copied")
class CBOptimizationDPP():
    ...
    def __init__(self, n_actions, dim, beta_sq, beta_lr):
        ...
        self.context = cp.Parameter(dim, id=0)
        ...
        # LinUCB constraints
        self.As_sqrt = [cp.Parameter((dim, dim), id=(100+i)) for i in range(n_actions)]
        self.Astheta_sq = [cp.Parameter(dim, id=(200+i)) for i in range(n_actions)]
        self.quadAstheta_sq = [cp.Parameter(id=(300+i)) for i in range(n_actions)]
        ...
        self.X_sum_sqrt = cp.Parameter((dim, dim), id=1)
        self.X_sum_theta_lr = [cp.Parameter(dim, id=(400+i)) for i in range(n_actions)]
        self.quadX_sum_theta_lr = [cp.Parameter(id=(500+i)) for i in range(n_actions)]
...
class CBOptimizationDPP_logonly():
    # to ensure DPP, replace quad_form with the following:
    # (theta - thetah)^T A (theta - thetah) = |A_sqrt theta|^2 - 2*theta^T (A @ thetah) + (thetah^T A thetah)
    def __init__(self, n_actions, dim, beta_lr):
        self.n_actions = n_actions
        self.theta = cp.Variable((n_actions, dim))
        self.context = cp.Parameter(dim, id=0)
        self.objectives_max = [cp.Maximize(cp.matmul(self.theta[a], self.context)) for a in range(n_actions)]
    ...
    def solve(self, context, theta_lr, X_sum, X_sum_sqrt, action, ucb=True):
    ...
    def solve_allactions(self, context, theta_lr, X_sum, X_sum_sqrt, 
                         ucb=True, multithreading=True):
        def solve_a(a):
            return self.solve(context, theta_lr, X_sum, X_sum_sqrt, a, ucb=ucb)
